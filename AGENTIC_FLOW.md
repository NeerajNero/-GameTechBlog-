...
## Project Override: Gaming + Tech Blog

This repository is now adapted for a content-first gaming + tech blog MVP. The reusable flow still applies, but the default full-stack bootstrap does not.

For this project:

- Use the static content-site lane.
- Read `docs/project-brief.md`, `docs/architecture.md`, and `docs/features/mvp-scope.md` first.
- Articles live as `.mdx` files under `content/articles/`.
- Skip Docker, Prisma, NestJS, backend, database, auth, admin panel, CMS, payments, user accounts, OpenAPI, SDK, and mobile setup unless a future approved decision changes scope.
- Skip `/api-integrate` for normal MVP static-site work.
